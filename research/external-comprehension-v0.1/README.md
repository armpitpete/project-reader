...
Use it for five real sessions with anonymous participant IDs `R01` through `R05`. Do not commit completed participant records unless Merrin has separately decided that the specific anonymous record is safe to keep in Git. The safer default is to copy a blank record template outside the repository, complete it there, and summarize only the aggregate findings.
...
- `FACILITATOR_GUIDE.md` - neutral 10-15 minute session guide.
...
- `SCORING_AND_DECISION_METHOD.md` - pre-session thresholds and decisions.
