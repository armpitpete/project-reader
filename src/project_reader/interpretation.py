...
_SHA = re.compile('^[0-9a-f]{40}$')
_REPOSITORY = re.compile('^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$')
_TASK = re.compile('^\\s*[-*]\\s+\\[(?P<mark>[ xX])\\]\\s+(?P<label>.+?)\\s*$')
_PURPOSE_ACTION = re.compile('\\b(help|helps|helping|allow|allows|enable|enables|let|lets|provide|provides|offer|offers|create|creates|build|builds|track|tracks|collect|collects|explain|explains|turn|turns|generate|generates|manage|manages|report|reports|analyse|analyses|analyze|analyzes)\\b', re.IGNORECASE)
_PURPOSE_NOUN = re.compile('\\b(tool|app|application|service|library|project|system|platform|engine|website|dashboard|repository|repositories|workflow|reader|collector)\\b', re.IGNORECASE)
_PURPOSE_NEGATIVE = re.compile('\\b(support|sponsor|sponsorship|donate|donation|fund|funding|copyright|licen[cs]e|warranty|disclaimer|security|contribut|warning|caution|notice|ko-fi|patreon)\\b', re.IGNORECASE)
...
def _validate_queue(data: dict[str, Any], name: str, segment: str) -> None:
    ...
    for item in data[name]:
        ...
        if url != expected:
            raise InterpretationError(f'The evidence bundle contains an uninspectable {name} URL')

def _validate(data: dict[str, Any]) -> None:
    if data.get('schema_version') != 1:
        raise InterpretationError('Automatic Interpretation Contract v0.4 requires evidence schema version 1')
    ...
    if data.get('repository_url') != f'https://github.com/{repository}':
        raise InterpretationError('The evidence bundle has no supported public repository URL')
    if not isinstance(data.get('source_commit'), str) or not _SHA.fullmatch(data['source_commit']):
...
def _paragraphs(markdown: str) -> tuple[str, ...]:
    ...
    for raw in markdown.splitlines():
        ...
        if fenced:
            continue
        excluded = not line or line.startswith(('#', '|', '>')) or bool(re.match('^[-*+]\\s+', line)) or bool(re.match('^\\d+[.)]\\s+', line))
        if excluded:
            if current:
    ...
    if current:
        paragraphs.append(' '.join(current))
    return tuple((value for raw in paragraphs if 20 <= len((value := _plain(raw))) <= 500))
...
def _purpose_score(text: str) -> int:
    ...
    if _PURPOSE_NOUN.search(text):
        score += 3
    if re.search('\\b(for|to)\\b', text, re.IGNORECASE):
        score += 2
    if _PURPOSE_NEGATIVE.search(text):
        score -= 8
    return score

def _purpose_paragraph(markdown: str, *, prefer_lead: bool) -> str | None:
    candidates = _paragraphs(markdown)
    if prefer_lead:
        lead_lines: list[str] = []
        ...
        lead = _paragraphs('\n'.join(lead_lines))
        if any((_purpose_score(text) >= 4 for text in lead)):
            candidates = lead
    ranked = [(_purpose_score(text), -index, text) for index, text in enumerate(candidates)]
    if not ranked:
        return None
...
def _source_url_matches(repository: str, source_commit: str, path: str, source_url: Any) -> bool:
    ...
    if parsed.hostname in {'github.com', 'www.github.com'}:
        ...
        return False
    return url_owner.casefold() == owner.casefold() and url_name.casefold() == name.casefold() and (url_commit == source_commit) and ('/'.join(url_path) == path)

def _files(data: dict[str, Any]) -> tuple[dict[str, dict[str, Any]], tuple[EvidenceReference, ...], list[Conflict], list[CandidateStatement]]:
    source_commit = data['source_commit']
    ranks = {item.get('path'): item.get('authority_rank') for item in data['progress_records'] if isinstance(item, dict) and isinstance(item.get('path'), str)}
    ...
    for index, raw in enumerate(data['important_files']):
        ...
        exact = commit == source_commit and _source_url_matches(data['repository'], source_commit, path, url)
        reason = None
        ...
        if raw.get('truncated') is True:
            unknowns.append(CandidateStatement(key=f'uncertainty:truncated:{index}', topic='evidence', text=f'Only part of {path} is present in the evidence bundle.', kind=StatementKind.UNKNOWN, basis='The collector marked this file as truncated.', evidence_keys=(_key(path),)))
    return (usable, tuple(sorted(references, key=lambda item: item.path.casefold())), conflicts, unknowns)

def _queue_references(data: dict[str, Any]) -> tuple[tuple[EvidenceReference, ...], tuple[str, ...]]:
    references: list[EvidenceReference] = []
    keys: list[str] = []
    for name, kind, segment, role in (('open_issues', 'issue', 'issues', 'open_issue'), ('open_pull_requests', 'pull_request', 'pull', 'open_pull_request')):
        for item in data[name]:
            number = item['number']
...
def _purpose(files: dict[str, dict[str, Any]]) -> CandidateStatement:
    ...
    for path in order:
        ...
        if raw.get('truncated') is True:
            continue
        if path != 'README.md' and raw.get('role') not in {'project_overview', 'project_status'}:
            continue
        if isinstance(raw.get('content'), str) and (text := _purpose_paragraph(raw['content'], prefer_lead=path == 'README.md')):
            return CandidateStatement(key='purpose', topic='purpose', text=text, kind=StatementKind.FACT if path == 'README.md' else StatementKind.INTERPRETATION, basis='The repository states this in README.md.' if path == 'README.md' else f'This is the strongest purpose-like statement in {path}.', evidence_keys=(_key(path),))
    return CandidateStatement(key='purpose', topic='purpose', text='The repository does not state a clear project purpose in the collected evidence.', kind=StatementKind.UNKNOWN, basis='No complete, purpose-like plain-language paragraph was found.')

def _authorities(files: dict[str, dict[str, Any]], records: list[Any]) -> tuple[AuthorityRecord, ...]:
    items = [item for item in records if isinstance(item, dict) and item.get('path') in files and isinstance(item.get('authority_rank'), int)]
    if not items:
        return ()
    primary = min((item['authority_rank'] for item in items))
    result: list[AuthorityRecord] = []
    for item in sorted(items, key=lambda value: (value['authority_rank'], value['path'].casefold())):
        path = item['path']
        declared = None
        ...
        result.append(AuthorityRecord(path=path, kind=str(item.get('kind', 'unknown')), authority_rank=item['authority_rank'], source_url=str(files[path].get('source_url', '')), source_commit=str(files[path].get('source_commit', '')), valid=item.get('valid_json') if path.endswith('.json') else True, primary=item['authority_rank'] == primary, declared_authority=declared))
    return tuple(result)

def _machine(path: str, raw: dict[str, Any], rank: int) -> tuple[list[_Work], list[CandidateStatement]]:
    try:
        data = json.loads(str(raw.get('content', '')))
    except json.JSONDecodeError:
        return ([], [CandidateStatement(key=f'uncertainty:invalid-json:{path}', topic='owner_authority', text=f'{path} is not valid JSON, so its work claims were not interpreted.', kind=StatementKind.UNKNOWN, basis='The owner-authority record could not be parsed.', evidence_keys=(_key(path),), owner_authority=True)])
    if not isinstance(data, dict) or not isinstance(data.get('stages'), list):
        return ([], [CandidateStatement(key=f'uncertainty:no-stages:{path}', topic='owner_authority', text=f'{path} does not contain a readable stages list.', kind=StatementKind.UNKNOWN, basis='The expected owner-authority structure is absent.', evidence_keys=(_key(path),), owner_authority=True)])
    work: list[_Work] = []
    unknowns: list[CandidateStatement] = []
    for index, stage in enumerate(data['stages']):
        ...
        numbers = all((isinstance(value, (int, float)) and (not isinstance(value, bool)) for value in (completed, total)))
...
def _work(files: dict[str, dict[str, Any]], authorities: tuple[AuthorityRecord, ...]) -> tuple[tuple[CandidateStatement, ...], tuple[CandidateStatement, ...], list[Conflict], list[CandidateStatement]]:
    if not authorities:
        return ((), (), [], [CandidateStatement('uncertainty:done-authority', 'done', 'Completed work is unknown because no recognised owner-authority record is present.', StatementKind.UNKNOWN, 'Open issues and activity are not completion evidence.'), CandidateStatement('uncertainty:remaining-authority', 'remaining', 'Remaining work is unknown because no recognised owner-authority record is present.', StatementKind.UNKNOWN, 'Open queues are not automatically the project finish line.')])
    ...
    for authority in authorities:
        ...
        if raw.get('truncated') is True:
            incomplete = True
            unknowns.append(CandidateStatement(f'uncertainty:truncated-authority:{authority.path}', 'owner_authority', f'{authority.path} is truncated, so its work items were not interpreted.', StatementKind.UNKNOWN, 'A partial owner-authority record cannot safely define done or remaining work.', (_key(authority.path),), True))
        elif authority.kind == 'machine_progress' or authority.path.endswith('.json'):
            found, more = _machine(authority.path, raw, authority.authority_rank)
            observations.extend(found)
            unknowns.extend(more)
        elif isinstance(raw.get('content'), str):
            observations.extend(_markdown(authority.path, raw['content'], authority.authority_rank))
...
def _technologies(files: dict[str, dict[str, Any]]) -> tuple[tuple[CandidateStatement, ...], list[CandidateStatement]]:
    ...
    def add(name: str, path: str, basis: str) -> None:
        found.setdefault(name, CandidateStatement('technology:' + re.sub('[^a-z0-9]+', '-', name.casefold()).strip('-'), 'technology', f'This repository uses {name}.', StatementKind.FACT, basis, (_key(path),)))
    for path, raw in files.items():
        ...
        if lower == 'pyproject.toml':
            ...
            if isinstance(project, dict):
                ...
                add('Python', path, f'pyproject.toml declares requires-python {version}.' if isinstance(version, str) else 'pyproject.toml defines a Python project.')
            ...
            if isinstance(package, dict):
                add('JavaScript or Node.js', path, 'package.json defines the JavaScript package.')
                dependencies = {name.casefold() for field in ('dependencies', 'devDependencies') if isinstance(package.get(field), dict) for name in package[field]}
                if 'typescript' in dependencies:
                    add('TypeScript', path, 'package.json lists TypeScript as a dependency.')
    ...
    return ((), [CandidateStatement('uncertainty:technology', 'technology', 'The collected evidence does not contain a recognised technology manifest.', StatementKind.UNKNOWN, 'Technology claims are refused unless a collected repository file directly supports them.')])
